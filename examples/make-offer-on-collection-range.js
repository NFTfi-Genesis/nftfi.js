...
async function run() {
  ...
  // Define range on collection to make offer on
  const nft = {
    address: process.env.NFTFI_SDK_EXAMPLE_NFT_ADDRESS,
    ids: {
      from: process.env.NFTFI_SDK_EXAMPLE_NFT_FROM,
      to: process.env.NFTFI_SDK_EXAMPLE_NFT_TO
    }
  };
  ...
  const result = await nftfi.offers.create({ type, terms, nft });
  if (result.errors) {
    ...
    console.log(`[INFO] made ranged collection offer on ${nft.address} with range ${nft.ids.from} to ${nft.ids.to}`);
    console.log(
      `[INFO] tokenIds: ${JSON.stringify(
        result.result.nft.ids
      )} duration: ${duration} days; principal: ${principal} ${ticker}; repayment: ${repayment} ${ticker}; APR: ${apr}%`
    );
    const validation = await nftfi.offers.validate({ offer: result.result });
    console.log('[INFO] validation result:', validation);
  }
}
